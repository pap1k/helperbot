...
function main()
    ...
    sampAddChatMessage(u8:decode("{e9ab4f}[HH]{5fdbea} HelperBot Loaded. Ебаш."), -1)
...
function se.onServerMessage(color, text)
	if fastinfo and text:find(u8:decode('{F5DEB3}Имя: .* Телефон: .* Проживает в: .*')) then
		local country = string.match(text, u8:decode('{F5DEB3}Имя: {ffffff}.*{F5DEB3} Телефон: {ffffff}.*{F5DEB3} Проживает в: {ffffff}(%a+){F5DEB3}%.'))
	...
	if fastinfo and text:find(u8:decode('Телефонный справочник')) then
		return false
	end
	if fastinfo and text:find(u8:decode('не найден в телефонном справочнике')) then
		sampAddChatMessage("[INFO]: {"..dectohex(sampGetPlayerColor(id))..'}'..sampGetPlayerNickname(id)..', {e64c5a}'..sampGetPlayerScore(id)..u8:decode('{5fdbea} LVL, {e64c5a}Нет данных по стране проживания'), 0x5fdbea)
		fastinfo = false
		return false
	end
	...
	if color == -5631489 and (settings.get("useFastAnsw") or settings.get("useAnswId") or settings.get("useColor")) then
		...
		if text:find(u8:decode('От .* для .*')) then
...
function findAnswer(inputstr, id)
    local work = true
    while work do
        local result = search(u8(lower(inputstr:gsub(u8:decode("[ёЁ]"), u8:decode("e")))))
        if result == "!!" then
            work = false
...
function announceAndSave(result, id)
    sampAddChatMessage(u8:decode('Возможно, подойдет ответ {c10013}')..result, 0x35b74a)
    answer.status = true
    answer.answ = result
    answer.id = id
    answer.ts = os.clock()
